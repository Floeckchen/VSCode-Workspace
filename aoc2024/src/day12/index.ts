...
class solver {
  ...
  private direction : Array<[number, number]> = [[-1, 0], [1, 0], [0, -1], [0, 1]];
  ...
  innerCorner(grid: string[][], row: number, col: number, plant: string) : number {
    ...
    const topRightPlant = (row - 1 >= 0 && col + 1 < grid[row].length) ? grid[row - 1][col + 1] : "";
    const topLeftPlant = (row - 1 >= 0 && col - 1 >= 0) ? grid[row - 1][col - 1] : "";
    const bottomRightPlant = (row + 1 < grid.length && col + 1 < grid[row].length) ? grid[row + 1][col + 1] : "";
    const bottomLeftPlant = (row + 1 < grid.length && col - 1 >= 0) ? grid[row + 1][col - 1] : "";
    ...
    if ( bottomLeftPlant != plant && bottomPlant == plant && leftPlant == plant) {
      corners++;
    }
    if ( bottomRightPlant != plant && bottomPlant == plant && rightPlant == plant) {
      corners++;
    }
  ...
  outerCorner(grid: string[][], row: number, col: number, plant: string) : number {
    const topPlant = (row - 1 >= 0) ? grid[row - 1][col] : "";
    const leftPlant = (col - 1 >= 0) ? grid[row][col - 1] : "";
    const rightPlant = (col + 1 < grid[row].length) ? grid[row][col + 1] : "";
    const bottomPlant = (row + 1 < grid.length) ? grid[row + 1][col] : "";
  ...
  getPosts(grid: string[][], row: number, col: number, plant: string) : number {
    return this.innerCorner(grid, row, col, plant) + this.outerCorner(grid, row, col, plant);
  }

  calculateRegion(grid: string[][], visited: boolean[][], row: number, col: number, plant: string, calcWholeFence: boolean = true) : number[] {
    ...
    for (let dir = 0; dir < 4; dir++) {
      ...
      const [f, a] = this.calculateRegion(grid, visited, newI, newJ, plant, calcWholeFence);
  ...
  part1() {
    ...
    for (let row = 0; row < grid.length; row++) {
      for (let col = 0; col < grid[row].length; col++) {
        cost.push( this.calculateRegion(grid, visited, row, col, grid[row][col]) );
      }
    }
  ...
  part2() {
    ...
    for (let row = 0; row < grid.length; row++) {
      for (let col = 0; col < grid[row].length; col++) {
        cost.push( this.calculateRegion(grid, visited, row, col, grid[row][col], false) );
      }
    }
