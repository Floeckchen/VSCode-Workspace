...
class solver {
  ...
  part1() {
    ...
    while (position_start < position_end && index_start <= index_end) {
      ...
      while (empty[index_start] != 0 && position_start < position_end && index_start < index_end) {               
  ...
  part2() {
    ...
    return harddriveDetails.split('').map( (x, i) => { return ( ( Math.max( x.charCodeAt(0) - 33, 0 ) )  * i )  }).reduce(sum);
