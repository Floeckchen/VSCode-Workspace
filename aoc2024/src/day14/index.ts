...
class roboter {
  ...
  constructor(input: string, dimensions: [number, number]) {
    [this.position, this.velocity] = input.split(" ").map(
      (x) =>
        x
          .split(/[^0-9-]+/)
          .map(Number)
          .splice(1) as [number, number],
    );
  ...
  getQuadrant(): number {
    // bottom right
    if (
      this.position[0] > Math.floor(this.dimensions[0] / 2) &&
      this.position[1] > Math.floor(this.dimensions[1] / 2)
    ) {
      return 1;
    }
    // bottom left
...
class solver {
  ...
  getGrid(robots: roboter[]): string[] {
    const grid = Array.from({ length: 103 }, () =>
      Array.from({ length: 101 }, () => "."),
    );
    robots.forEach(
      (robot) => (grid[robot.position[1]][robot.position[0]] = "#"),
    );
