...
class solver {
  ...
  howManyXmasStartInLine(line: string, i: number, lines: string[]): number {
    return line
      .split("")
      .map((c, j) => this.howManyXmasStartHere(i, j, lines))
      .reduce((m, n) => m + n);
  }

  howManyCrossedMasStartInLine(
    line: string,
    i: number,
    lines: string[],
  ): number {
    return line
      .split("")
      .map((c, j) => this.howManyCrossedMasStartHere(i, j, lines))
      .reduce((m, n) => m + n);
  }

  howManyXmasStartHere(i: number, j: number, lines: string[]): number {
    if (lines[i][j] != "X") return 0;
  ...
  part1() {
    ...
    return lines
      .map((line, i, lines) => this.howManyXmasStartInLine(line, i, lines))
      .reduce((m, n) => m + n);
  }
  ...
  part2() {
    ...
    return lines
      .map((line, i, lines) =>
        this.howManyCrossedMasStartInLine(line, i, lines),
      )
      .reduce((m, n) => m + n);
  }
}
